...
class SetPoint():
    ...
    def publish(self, client, setPoint):
        ...
        status = result[0]
        if status == 0:
            print(f"Succesfully send '{msg}' to topic '{self.topic}' P")
        else:
            print(f"Fail to send '{msg}' to topic '{self.topic}' P")
            raise Exception(f"Fail to send '{msg}' to topic '{self.topic}' P")
